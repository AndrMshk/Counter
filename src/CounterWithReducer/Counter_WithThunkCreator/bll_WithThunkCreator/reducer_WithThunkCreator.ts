...
import { AppStateType } from './store_WithThunkCreator';
...
export const incValueTC = () => (dispatch: Dispatch, gesState: () => AppStateType) => {
  const currentValue = gesState().counter.value;
  localStorage.setItem('counter-value', JSON.stringify(currentValue + 1));  // bad decision
  dispatch(incAC());
};

export const setFromLocalStorageTC = () => (dispatch: Dispatch) => {
  const valueFromLS = localStorage.getItem('counter-value');
  if (valueFromLS) {
